...
class TrendFollowingStrategyV3(IStrategy):
    """
    Trend-Following Strategy V3 - Sortino Optimized
    
    Optimized Parameters (2025-11-28):
    - EMA Fast/Slow: 90/207
    - ADX Threshold: 19
    ...
    # --- EMA Parameters ---
    ema_fast_period = IntParameter(20, 100, default=90, space='buy', optimize=True)
    ema_slow_period = IntParameter(100, 250, default=207, space='buy', optimize=True)
    
    # --- ADX Parameters ---
    adx_period = IntParameter(10, 20, default=17, space='buy', optimize=True)
    adx_threshold = IntParameter(18, 30, default=19, space='buy', optimize=True)
    adx_exit_threshold = IntParameter(12, 20, default=20, space='sell', optimize=True)
    
    # --- ATR Parameters ---
    atr_period = IntParameter(10, 20, default=12, space='buy', optimize=True)
    atr_stop_multiplier = DecimalParameter(1.0, 2.5, default=1.0, decimals=1, space='sell', optimize=True)
    
    # --- Volume Parameters ---
    volume_ma_period = IntParameter(15, 30, default=16, space='buy', optimize=True)
    volume_threshold = DecimalParameter(1.0, 1.5, default=1.1, decimals=1, space='buy', optimize=True)
    
    # --- Pullback Parameters ---
    pullback_atr_multiplier = DecimalParameter(0.5, 2.0, default=1.3, decimals=1, space='buy', optimize=True)
    
    # --- EMA Distance Filter ---
    ema_distance_threshold = DecimalParameter(0.001, 0.01, default=0.003, decimals=3, space='buy', optimize=True)
    
    # --- Take Profit Parameters ---
    tp1_risk_reward = DecimalParameter(0.8, 1.5, default=0.9, decimals=1, space='sell', optimize=True)
    tp2_risk_reward = DecimalParameter(1.5, 3.5, default=3.4, decimals=1, space='sell', optimize=True)
    tp1_close_percentage = DecimalParameter(0.3, 0.7, default=0.4, decimals=1, space='sell', optimize=True)
    
    # --- Trailing Stop Parameters ---
    use_trailing_stop = CategoricalParameter([True, False], default=True, space='sell', optimize=True)
    trailing_atr_multiplier = DecimalParameter(1.5, 3.0, default=2.8, decimals=1, space='sell', optimize=True)
    
    # --- Candlestick Filter Parameters ---
    min_candle_body_ratio = DecimalParameter(0.2, 0.5, default=0.2, decimals=1, space='buy', optimize=True)
    max_shadow_ratio = DecimalParameter(0.5, 0.8, default=0.7, decimals=1, space='buy', optimize=True)
    
    # --- Volatility Filter Parameters ---
    min_atr_ratio = DecimalParameter(0.6, 1.0, default=0.8, decimals=1, space='buy', optimize=True)
    max_atr_spike = DecimalParameter(2.0, 3.0, default=2.4, decimals=1, space='buy', optimize=True)
    
    # --- Time-Based Exit ---
    max_trade_duration_hours = IntParameter(24, 96, default=63, space='sell', optimize=True)
