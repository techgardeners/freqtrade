...
class SimplifiedICTStrategySafe(IStrategy):
    """
    Simplified ICT-Inspired Strategy (Safe Version)
    Author: Antigravity
    Version: 1.1.0
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["swing_high"] = (
            dataframe["high"]
            .rolling(window=swing_period * 2 + 1, center=False)
            .max()
            .shift(swing_period)
        )
        dataframe["swing_low"] = (
            ...
            if dataframe["recent_fvg_bull"].iloc[i] > 0:
                ...
                for j in range(i - 1, max(0, i - lookback), -1):
                    if dataframe["fvg_bullish"].iloc[j]:
                        ...
                        dataframe.loc[i, "active_fvg_bull_bottom"] = dataframe[
                            "fvg_bull_bottom"
                        ].iloc[j]
                        break
            ...
            if dataframe["recent_fvg_bear"].iloc[i] > 0:
                for j in range(i - 1, max(0, i - lookback), -1):
                    if dataframe["fvg_bearish"].iloc[j]:
                        ...
                        dataframe.loc[i, "active_fvg_bear_bottom"] = dataframe[
                            "fvg_bear_bottom"
                        ].iloc[j]
                        break
